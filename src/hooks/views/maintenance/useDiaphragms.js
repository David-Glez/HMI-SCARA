...
const useDiaphragms = () => {
    ...
    const pumps = settings.diaphragms;
    const [diafragmas, setDiafragmas] = useState([]);
    ...
    useEffect(() => {
        pumps.forEach((item) => {
            ...
            setDiafragmas(diafragmas => ([...diafragmas, data ]));
        })
    ...
    const onChangeRange = (e) => {
        ...
        setDiafragmas(diafragmas.map(item => item.name === name ? {...item, seconds:value} : item));
    }

    const sendToSerial = (e, name) => {
        e.preventDefault();
        const element = diafragmas.find(i => i.name == name);
